...
async function fetchStockAnalysisHistory(ticker, market) {
  ...
  if (!urls.length && market === "Nigeria") {
    urls.push(`https://stockanalysis.com/quote/ngx/${encodeURIComponent(String(ticker || "").trim().toUpperCase())}/history/`);
  }
...
async function fetchNgxPulsePriceHistory(ticker, market) {
  if (market !== "Nigeria") return null;
  const symbol = String(ticker || "").trim().toUpperCase();
  if (!symbol) return null;
  const url = `https://ngxpulse.ng/api/ngxdata/prices/${encodeURIComponent(symbol)}?days=400`;
  const data = await fetchJson(url, {
    referer: `https://ngxpulse.ng/stocks/${encodeURIComponent(symbol)}`,
  ...
  const rows = Array.isArray(data?.prices)
    ...
    : [];
  const history = dedupeAndSortHistory(rows);
  if (history.length < 15) return null;
  return {
    history,
    provider: "NGX Pulse public price history",
    sourceLabel: "NGX Pulse",
    sourceUrl: url
  };
}

async function fetchVerifiedRsiFallbackHistory(ticker, market) {
  const ngxPulse = await fetchNgxPulsePriceHistory(ticker, market);
  if (ngxPulse?.history?.length >= 15) return ngxPulse;
